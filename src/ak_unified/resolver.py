...
def _sort_providers_for(dataset_id: str, providers: List[ProviderSpec], adapters_pref: Optional[List[str]] = None) -> List[ProviderSpec]:
    items = list(providers)
    if adapters_pref and len(adapters_pref) > 0:
        adapter_order = [a.lower() for a in adapters_pref]
    else:
        adapter_order = [a.lower() for a in _provider_priority_for_dataset(dataset_id)]
    if adapter_order:
        items.sort(key=lambda p: (adapter_order.index(p.adapter.lower()) if p.adapter.lower() in adapter_order else 10**6,
                                  (p.priority if p.priority is not None else 10**6)))
    else:
        items.sort(key=lambda p: (p.priority if p.priority is not None else 10**6))
    ...
    for adapter, plist in grouped.items():
        vorder = _vendor_priority_for_dataset(dataset_id, adapter)
        if vorder:
...
def resolve_providers(dataset_id: str, *, adapter: Optional[List[str]] = None) -> List[ProviderSpec]:
    ...
    if adapter and len(adapter) > 0:
        req = [a.lower() for a in adapter]
        filtered = [p for p in providers if p.adapter.lower() in req]
        if not filtered:
            supported = sorted({p.adapter for p in providers})
            raise ValueError(f"Unsupported adapter(s) for dataset '{dataset_id}': {adapter}. Supported: {supported}")
        return _sort_providers_for(dataset_id, filtered, adapters_pref=adapter)

    return _sort_providers_for(dataset_id, providers, adapters_pref=None)